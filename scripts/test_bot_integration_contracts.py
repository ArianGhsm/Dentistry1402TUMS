...
DELIVERY_STORE = (ROOT / "public_html/api/bot_delivery_store.php").read_text(encoding="utf-8")
...
require("max(6, (int) ($normalized['schemaVersion'] ?? 0))" in STORE, "legacy bot stores must migrate to generic-payment return-route schema v6")
...
require("dent_bot_payment_delivery_store_with_lock" in DELIVERY_STORE and "dent_bot_queue_payment_success_deliveries" in STORE and "hash_hmac('sha256', $dedupeKey" in STORE, "verified payment delivery must be durable and idempotent")
require("finalize-delivery-store-split" in DELIVERY_STORE and "identity-store-compaction-deferred" in DELIVERY_STORE, "identity and delivery queues must have independent persistence lifetimes")
...
require("ackPaymentResultDeliveriesV1" in STORE and "ack-payment-results-batch" in STORE, "payment-result acknowledgements must be committed in bounded batches")
