...
payments_with_store_lock(static function (array &$store): void {
    // Deliberately use a managed key that differs from the provider name. The
    // Voice bridge must resolve the unique enabled Zibal record by provider
    // semantics instead of assuming that every deployment calls it "zibal".
    $store['gatewaySettings'] = ['managed' => true, 'default' => 'voice-zibal-primary'];
    $store['gateways'] = [[
        'id' => 2, 'key' => 'voice-zibal-primary', 'provider' => 'zibal', 'label' => 'Synthetic Zibal',
