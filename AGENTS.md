...
- داده و فایل‌های سیستم مشاهده خصوصی جزوات باید در storage خصوصی `private_notes/` بماند: metadata در `private_notes/store.json`، PDF اصلی فقط در `private_notes/originals/` و خروجی رندر/tile فقط در `private_notes/pages/`. هیچ‌کدام نباید زیر `public_html/` یا هاست دانلود عمومی قرار بگیرند و deploy نباید PDF اصلی، tileها، jobها، رویدادهای مشاهده، دستگاه‌ها، تعلیق‌ها یا security eventها را reset یا public کند.
