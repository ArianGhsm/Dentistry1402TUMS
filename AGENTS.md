...
- داده و فایل‌های سیستم مشاهده خصوصی جزوات باید در storage خصوصی `private_notes/` بماند: metadata در `private_notes/store.json`، PDF اصلی فقط در `private_notes/originals/` و خروجی رندر/tile فقط در `private_notes/pages/`. تحویل tile باید از API backend با token کوتاه‌مدت، session/device معتبر و watermark server-side انجام شود؛ cache کوتاه‌مدت watermark فقط زیر `server-only/tmp/private_notes_tile_cache/` مجاز است. هیچ‌کدام نباید زیر `public_html/` یا هاست دانلود عمومی قرار بگیرند و deploy نباید PDF اصلی، tileها، jobها، رویدادهای مشاهده، دستگاه‌ها، تعلیق‌ها یا security eventها را reset یا public کند.
...
- اگر اعلان shared قابلیت زمان‌بندی، پیامک، حذف سراسری، وضعیت مشاهده‌کنندگان/ندیده‌ها، snooze، digest روزانه، reminderهای فرم/پرداخت/آزمون یا preferenceهای نوع اعلان بگیرد، همین stateها هم باید در همان storage و API مشترک اعلان‌ها بمانند و برای SMS فقط از سرویس/تنظیمات مشترک `auth_store` استفاده شود؛ queue، snooze-state، digest-state یا read-state موازی برای هر صفحه/قابلیت مجاز نیست.
...
- کارت‌های `/app/` برای منابع، فرم‌ها، نمرات، نوید و خرید باید از service catalog/feature flags همان cohort ساخته شوند؛ اضافه‌شدن cohort جدید نباید نیازمند copy-paste کارت یا `data-main-student-only`/`data-prosthesis-only` جدید در HTML خانه باشد.
