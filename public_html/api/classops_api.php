...
function classops_api_object_field(array $payload, string $field, bool $required = true): array
{
    ...
    $value = $payload[$field] ?? [];
    if (is_string($value)) {
        try { $decoded = json_decode($value, true, CLASSOPS_API_MAX_DEPTH, JSON_THROW_ON_ERROR); }
        catch (JsonException $exception) { classops_domain_error('CLASSOPS_INVALID_OBJECT', "فیلد {$field} باید object معتبر باشد."); }
        $value = $decoded;
    }
    if (!is_array($value) || array_is_list($value)) classops_domain_error('CLASSOPS_INVALID_OBJECT', "فیلد {$field} باید object معتبر باشد.");
    return $value;
}
...
{
    if (dent_request_method() !== $method) classops_domain_error('CLASSOPS_METHOD_NOT_ALLOWED', 'متد این عملیات معتبر نیست.', 405);
}
...
function classops_api_student_for_read(): array
{
    ...
    if (classops_stage2_is_owner($viewer)) classops_domain_error('CLASSOPS_STUDENT_SCOPE_REQUIRED', 'این endpoint مخصوص projection شخصی دانشجو است.', 403);
...
function classops_api_stage2_status(): array
{
    ...
    return [
        ...
        'capabilities' => classops_stage2_capabilities(),
    ];
}
...
function classops_api_audience_preview(array $owner, array $payload): array
{
    ...
    return ['success'=>true,'preview'=>classops_audience_preview($resolution, null, true),'resolution'=>$resolution,'mutationPerformed'=>false];
}
...
function classops_api_reminder_preview(array $owner, array $payload): array
{
    ...
    if (!is_array($policy)) return ['success'=>true,'policy'=>null,'plan'=>['ok'=>true,'intents'=>[],'supersessions'=>[]],'mutationPerformed'=>false];
    $item = $preview['item'];
    $resolution = $preview['audienceResolution'];
    $plannerItem = [
        'itemId'=>'cop_'.str_repeat('c',24),'revision'=>1,'itemType'=>(string)$item['type'],'status'=>'active',
        ...
        'deliveryPolicyRef'=>'delivery_policy_preview','reminderPolicy'=>$policy,'serviceRef'=>$preview['serviceRef']??null,
    ];
    $plan = classops_reminder_plan([
        'contractVersion'=>'classops-reminder-v1','planningHorizonSeconds'=>CLASSOPS_REMINDER_MAX_HORIZON_SECONDS,
        'maxOccurrences'=>64,'items'=>[$plannerItem],'knownOccurrences'=>[],
    ]);
    return ['success'=>true,'policy'=>$policy,'plan'=>$plan,'mutationPerformed'=>false];
}

function classops_api_owner_task_state(array $owner): array
{
    ...
    if (classops_stage2_item_audience_for_student($item, $student) === null) classops_domain_error('CLASSOPS_TASK_STUDENT_NOT_ELIGIBLE', 'دانشجو عضو audience این revision نیست.', 403);
...
try {
    ...
    if ($action === 'list') {
        ...
        dent_json_response(['success'=>true,'data'=>classops_list_items([
            'cohortKey'=>$cohort,'type'=>(string)($_GET['type']??''),'status'=>(string)($_GET['status']??''),
            'limit'=>$_GET['limit']??25,'cursor'=>(string)($_GET['cursor']??''),
        ])]);
    }
    if ($action === 'get') {
        classops_api_require_method('GET');
        ...
        dent_json_response(['success'=>true,'item'=>$item,'audience'=>$aud===null?null:['resolutionHash'=>$aud['resolutionHash'],'recipientCount'=>count($aud['snapshot']['recipientStudentNumbers']??[])]]);
    }
    if ($action === 'revisions') {
        classops_api_require_method('GET');
        classops_api_owner_for_read();
        ...
        dent_json_response(['success'=>true,'data'=>classops_revision_history(
            classops_stage2_require_item_id($_GET['id'] ?? ''),(int)($_GET['limit']??50),(string)($_GET['cursor']??'')
        )]);
    }
    if ($action === 'task-state') {
        ...
        dent_json_response(classops_api_owner_task_state($owner));
    }
    if ($action === 'ack-stats') {
        classops_api_require_method('GET');
        $owner = classops_api_owner_for_read();
    ...
    if ($action === 'student-list') {
        ...
        dent_json_response(['success'=>true,'data'=>classops_stage2_student_list($student,['type'=>(string)($_GET['type']??''),'status'=>(string)($_GET['status']??'')])]);
    }
    ...
    if ($action === 'tomorrow-summary' || $action === 'weekly-digest') {
        ...
        dent_json_response(['success'=>true,'digest'=>classops_stage2_digest($viewer,$action==='tomorrow-summary'?'tomorrow':'weekly')]);
    }
    ...
    if ($action === 'ai-draft-create') {
        ...
        $forwarded=array_key_exists('forwardedText',$payload)&&$payload['forwardedText']!==null?(string)$payload['forwardedText']:null;
        dent_json_response(classops_stage2_ai_create($owner,(string)($payload['ownerText']??''),$forwarded,trim((string)($payload['cohortKey']??''))));
    }
    if ($action === 'ai-draft-edit') {
        $owner=classops_api_owner_for_post(true);
        classops_assert_known_keys($payload,['action','priorDraft','ownerEditText']);
        dent_json_response(classops_stage2_ai_edit($owner,classops_api_object_field($payload,'priorDraft'),(string)($payload['ownerEditText']??'')));
    }
    ...
    if ($action === 'preview' || $action === 'delivery-preview') {
        $owner=classops_api_owner_for_post();
        $copy=$payload; unset($copy['action']);
        $preview=classops_stage2_preview($owner,$copy);
        dent_json_response($action==='delivery-preview' ? ['success'=>true,'destinations'=>$preview['destinations'],'audience'=>$preview['audience'],'confirmation'=>$preview['confirmation'],'mutationPerformed'=>false] : $preview + ['mutationPerformed'=>false]);
    }
    if ($action === 'reminder-preview') {
        $owner=classops_api_owner_for_post();
        dent_json_response(classops_api_reminder_preview($owner,$payload));
    }
    if (in_array($action,['confirm','create','update'],true)) {
        $owner=classops_api_owner_for_post();
        $copy=$payload; unset($copy['action']);
        if ($action==='create') $copy['mode']='create';
        if ($action==='update') $copy['mode']='update';
    ...
    if ($action === 'cancel' || $action === 'archive') {
        ...
        dent_json_response(['success'=>true]+classops_stage2_cancel_or_archive(
            $owner,$action,classops_stage2_require_item_id($payload['id']??''),(int)($payload['expectedRevision']??0),
            trim((string)($payload['idempotencyKey']??'')),trim((string)($payload['reason']??''))
        ));
    }
    ...
    if ($action === 'student-service-transition') {
        $student=classops_api_student_for_post();
        $copy=$payload; unset($copy['action']);
        dent_json_response(['success'=>true,'state'=>classops_stage2_student_service_transition($student,$copy),'externallyVerified'=>false]);
    }
    ...
    classops_persistence_log('error',[
        'action'=>'api','decodeStatus'=>'not-applicable','commitResult'=>'unhandled-error','reasonCode'=>'CLASSOPS_INTERNAL_ERROR',
    ]);
