(function () {
  ...
  var chatUtils = window.Dent1402ChatUtils || {};
  var BLANK_AVATAR_DATA_URL = chatUtils.BLANK_AVATAR_DATA_URL;
  var asObject = chatUtils.asObject;
  var toText = chatUtils.toText;
  var escapeHtml = chatUtils.escapeHtml;
  var formatNavBadgeCount = chatUtils.formatNavBadgeCount;
  var normalizeSpace = chatUtils.normalizeSpace;
  var normalizeDigits = chatUtils.normalizeDigits;
  var normalizeStudentNumber = chatUtils.normalizeStudentNumber;
  var avatarLabel = chatUtils.avatarLabel;
  var snippet = chatUtils.snippet;
  var clamp = chatUtils.clamp;
  var toNumber = chatUtils.toNumber;
  var formatTime = chatUtils.formatTime;
  var formatClock = chatUtils.formatClock;
  var formatDate = chatUtils.formatDate;
  var formatDateTime = chatUtils.formatDateTime;
  var formatLastSeenLabel = chatUtils.formatLastSeenLabel;
  var dayKeyFromTimestamp = chatUtils.dayKeyFromTimestamp;
  var formatFileSize = chatUtils.formatFileSize;
  var formatDuration = chatUtils.formatDuration;
  var voiceSpeedLabel = chatUtils.voiceSpeedLabel;
  var stableHashSeed = chatUtils.stableHashSeed;
  var seededWaveformSamples = chatUtils.seededWaveformSamples;
  var normalizeAttachmentCategory = chatUtils.normalizeAttachmentCategory;
  var attachmentCategoryLabel = chatUtils.attachmentCategoryLabel;
  var isGeneratedMessagePlaceholder = chatUtils.isGeneratedMessagePlaceholder;
  var meaningfulMessageText = chatUtils.meaningfulMessageText;
  var normalizeAvatarUrl = chatUtils.normalizeAvatarUrl;
  ...
  var chatCache = window.Dent1402ChatCache.create({
    CHAT_DRAFT_SAVE_DELAY_MS: CHAT_DRAFT_SAVE_DELAY_MS,
    CHAT_DRAFT_TTL_MS: CHAT_DRAFT_TTL_MS,
    CHAT_FAST_CACHE_CONVERSATION_LIMIT: CHAT_FAST_CACHE_CONVERSATION_LIMIT,
    CHAT_FAST_CACHE_MESSAGE_LIMIT: CHAT_FAST_CACHE_MESSAGE_LIMIT,
    CHAT_FAST_CACHE_TTL_MS: CHAT_FAST_CACHE_TTL_MS,
    CHAT_FAST_CACHE_VERSION: CHAT_FAST_CACHE_VERSION,
    MAX_MESSAGE_SIZE: MAX_MESSAGE_SIZE,
    activeConversation: activeConversation,
    appendMessages: appendMessages,
    asObject: asObject,
    autosizeComposer: autosizeComposer,
    chatTextEl: chatTextEl,
    clearReplyTarget: clearReplyTarget,
    closeMentionSuggestions: closeMentionSuggestions,
    invalidateMessageListCache: invalidateMessageListCache,
    messageList: messageList,
    messagesEl: messagesEl,
    normalizeConversation: normalizeConversation,
    normalizeConversationListCategory: normalizeConversationListCategory,
    normalizeMessage: normalizeMessage,
    normalizeSpace: normalizeSpace,
    normalizeStudentNumber: normalizeStudentNumber,
    pageCohort: pageCohort,
    pauseAllVoiceNotes: pauseAllVoiceNotes,
    renderConversationList: renderConversationList,
    replaceConversations: replaceConversations,
    resetThreadSearchState: resetThreadSearchState,
    setConnectionState: setConnectionState,
    setThreadVisible: setThreadVisible,
    showStreamState: showStreamState,
    state: state,
    syncComposerDraftState: syncComposerDraftState,
    toNumber: toNumber,
    toText: toText,
    updateComposerState: updateComposerState,
    updateConversationFilterTabs: updateConversationFilterTabs,
    updateFabVisibility: updateFabVisibility,
    updateInfoSheet: updateInfoSheet,
    updateMessageSelectionUi: updateMessageSelectionUi,
    updateMobileNav: updateMobileNav,
    updateMuteUi: updateMuteUi,
    updatePinnedUi: updatePinnedUi,
    updateThreadHead: updateThreadHead,
    updateThreadSearchUi: updateThreadSearchUi
  });
  var chatStorage = chatCache.chatStorage;
  var chatCacheUserKey = chatCache.chatCacheUserKey;
  var chatCacheKey = chatCache.chatCacheKey;
  var chatDraftKey = chatCache.chatDraftKey;
  var compactCachedConversation = chatCache.compactCachedConversation;
  var compactCachedMessage = chatCache.compactCachedMessage;
  var clearFastChatCacheSaveHandle = chatCache.clearFastChatCacheSaveHandle;
  var saveFastChatCacheNow = chatCache.saveFastChatCacheNow;
  var scheduleFastChatCacheSave = chatCache.scheduleFastChatCacheSave;
  var readFastChatCache = chatCache.readFastChatCache;
  var hydrateFastChatCache = chatCache.hydrateFastChatCache;
  var readComposerDraft = chatCache.readComposerDraft;
  var saveComposerDraftNow = chatCache.saveComposerDraftNow;
  var scheduleComposerDraftSave = chatCache.scheduleComposerDraftSave;
  var clearComposerDraft = chatCache.clearComposerDraft;
  var restoreComposerDraftForConversation = chatCache.restoreComposerDraftForConversation;
  var clearThreadState = chatCache.clearThreadState;
  ...
  var realtimeTransport = window.Dent1402ChatRealtimeTransport.create({
    CHAT_PRESENCE_HEARTBEAT_MS: CHAT_PRESENCE_HEARTBEAT_MS,
    CHAT_STREAM_RETRY_MS: CHAT_STREAM_RETRY_MS,
    CHAT_TYPING_IDLE_MS: CHAT_TYPING_IDLE_MS,
    CHAT_TYPING_REFRESH_MS: CHAT_TYPING_REFRESH_MS,
    MAX_POLL_MS: MAX_POLL_MS,
    MIN_POLL_MS: MIN_POLL_MS,
    activeConversation: activeConversation,
    apiPost: apiPost,
    applyPresenceBundle: applyPresenceBundle,
    asObject: asObject,
    chatTextEl: chatTextEl,
    clamp: clamp,
    consumeUnauthorized: consumeUnauthorized,
    ensureSuccessResponse: ensureSuccessResponse,
    normalizeSpace: normalizeSpace,
    setConnectionState: setConnectionState,
    setConversationReadStateById: setConversationReadStateById,
    state: state,
    syncConversation: syncConversation,
    toNumber: toNumber
  });
  var clearStreamRetryTimer = realtimeTransport.clearStreamRetryTimer;
  var clearStreamSyncTimer = realtimeTransport.clearStreamSyncTimer;
  var clearPresenceHeartbeatTimer = realtimeTransport.clearPresenceHeartbeatTimer;
  var clearTypingTimers = realtimeTransport.clearTypingTimers;
  var configureTransport = realtimeTransport.configureTransport;
  var buildStreamContextUrl = realtimeTransport.buildStreamContextUrl;
  var stopRealtimeStream = realtimeTransport.stopRealtimeStream;
  var queueRealtimeSync = realtimeTransport.queueRealtimeSync;
  var scheduleRealtimeReconnect = realtimeTransport.scheduleRealtimeReconnect;
  var sendPresenceHeartbeat = realtimeTransport.sendPresenceHeartbeat;
  var schedulePresenceHeartbeat = realtimeTransport.schedulePresenceHeartbeat;
  var clearTypingActivity = realtimeTransport.clearTypingActivity;
  var handleComposerTypingActivity = realtimeTransport.handleComposerTypingActivity;
  var startRealtimeStream = realtimeTransport.startRealtimeStream;
  var refreshTransportBinding = realtimeTransport.refreshTransportBinding;
  var stopPolling = realtimeTransport.stopPolling;
  var startPolling = realtimeTransport.startPolling;
  var clearAutoReadTimer = realtimeTransport.clearAutoReadTimer;
  var scheduleAutoMarkRead = realtimeTransport.scheduleAutoMarkRead;
