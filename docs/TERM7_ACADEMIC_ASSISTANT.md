...
`1405-1406.4` است. عضویت گروه‌ها داخل کد یا PDF حدس زده نمی‌شود و فقط با شماره
...
برنامهٔ زمانی canonical فقط در `dent_term7_schedule()` تعریف می‌شود. طرح درس‌ها
هیچ timetable موازی نمی‌سازند و ساعت، روز، روتیشن یا مکان برنامه را از روی PDF
بازنویسی نمی‌کنند. فایل `public_html/api/classops_term7_syllabus.php` registry
مشترک طرح درس‌ها را روی occurrence موجود در ClassOps enrich می‌کند و دادهٔ
ساخت‌یافتهٔ PDFهای اصلاح‌شده در `classops_term7_syllabus_data.php` قرار دارد.
...
جلسهٔ مجازی یا آفلاین برچسب فارسی صریح دارد و چون ساعت مجازی مستقلی در منبع
ثبت نشده، در projection برای آن ساعت یا مکان فیزیکی آمفی‌تئاتر جعل نمی‌شود.
