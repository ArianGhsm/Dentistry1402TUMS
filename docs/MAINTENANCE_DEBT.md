...
- `public_html/assets/site/scripts/chat.js` (pure helpers in `chat-utils.js`; cache/draft persistence in `chat-cache-drafts.js`; SSE/polling/presence transport in `chat-realtime-transport.js`) — separate transport/state, message rendering, composer/media, and navigation concerns before adding substantial new chat behavior.
