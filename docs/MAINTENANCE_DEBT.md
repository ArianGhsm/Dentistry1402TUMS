...
- `public_html/assets/site/scripts/chat.js` (pure helpers in `chat-utils.js`; cache/draft persistence in `chat-cache-drafts.js`; SSE/polling/presence transport in `chat-realtime-transport.js`; the former 1,346-line event binder is split into six bounded domain binders) — separate transport/state, message rendering, composer/media, and navigation concerns before adding substantial new chat behavior.
...
- `public_html/assets/site/scripts/account.js` (pure helpers in `account-utils.js`; owner analytics in `account-owner-analytics.js`; notification hub/workflows in `account-notifications.js`) — separate authentication/OTP, profile, settings, payments, and account-linked product controllers.
