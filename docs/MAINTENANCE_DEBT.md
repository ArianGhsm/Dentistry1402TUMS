...
- `public_html/api/auth_store.php` — persistence primitives are in `auth_store_persistence.php` and SMS provider/configuration primitives are in `auth_store_sms.php`; continue separating OTP, identity/session, and account-domain operations while preserving the single canonical auth store.
