...
class ClassOpsSurfaceTests(unittest.TestCase):
    ...
    def test_owner_student_menu_separation(self) -> None:
        ...
        self.assertIn("draft.create", owner_actions)
        self.assertNotIn("draft.create", student_actions)
        self.assertNotIn("items.list", student_actions)
        self.assertIn("exam.view", student_actions)
        self.assertFalse(next(button for button in student.buttons if button.action == "exam.view").enabled)
    ...
    def test_stale_revision_has_explicit_conflict_state(self) -> None:
        ...
        self.assertIn("نسخه آیتم تغییر کرده", view.text)
    ...
    def test_ai_free_text_is_preview_only(self) -> None:
        ...
        self.assertIn("چیزی ارسال یا ذخیره نشده", view.text)
        intent = build_intent("ai.draft_request", OWNER, payload={"text": "نمونه"})
        self.assertIsNone(intent.idempotency_key)
        self.assertFalse(intent.confirmation_required)

    def test_critical_ack_is_student_only_and_disabled_without_backend(self) -> None:
        view = render_critical_ack(STUDENT, "اطلاعیه امتحان", 4, self.caps)
        self.assertEqual(view.state, "disabled")
        self.assertFalse(view.buttons[0].enabled)
        with self.assertRaises(ValueError):
            render_critical_ack(OWNER, "اطلاعیه امتحان", 4, self.caps)
        with self.assertRaises(SurfaceContractError):
            build_intent("exam.ack", OWNER, item_id="x", expected_revision=1, nonce="nonce-0001")

    def test_tomorrow_weekly_render_parity_and_capability_gate(self) -> None:
        disabled = render_summary("tomorrow", STUDENT, [], self.caps)
        self.assertEqual(disabled.state, "disabled")
        enabled_caps = dict(self.caps)
        enabled_caps["summary.tomorrow"] = Capability(True)
        enabled_caps["summary.weekly"] = Capability(True)
        rows = [{"title": "کلاس پریو", "when": "08:00"}]
        for kind in ("tomorrow", "weekly"):
            view = render_summary(kind, STUDENT, rows, enabled_caps)
    ...
    def test_no_raw_ids_or_secrets_in_payload(self) -> None:
        ...
        for payload in forbidden_payloads:
            with self.subTest(payload=payload), self.assertRaises(SurfaceContractError):
                build_intent("ai.draft_request", OWNER, payload=payload)

    def test_disabled_capability_never_becomes_semantic_action(self) -> None:
        view = build_menu(STUDENT, self.caps)
        rendered = TelegramAdapter().render(view)
        self.assertNotIn("exam.view", rendered.semantic_actions)
        disabled_callbacks = [row[0]["callback_data"] for row in rendered.keyboard["inline_keyboard"]]
        self.assertTrue(all(value == "classops:disabled" for value in disabled_callbacks))
